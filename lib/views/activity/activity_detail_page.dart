...
class ActivityDetailPage extends StatefulWidget {
  const ActivityDetailPage({Key? key, required this.activityOwned})
      : super(key: key);
...
class _ActivityDetailPageState extends State<ActivityDetailPage> {
  ...
  void initState() {
    ...
    fetchActivityDetails(widget.activityOwned);

    if (widget.activityOwned.status == 'wait ' ||
        widget.activityOwned.status == 'completed') {
          print('dafi');
  ...
  void fetchActivityDetails(ActivityOwned activityOwned) async {
    prov.isFetchingData = true;

    try {
      actDetails = await prov.fetchDetailsByActivityId(activityOwned.activity);
      // actDetails.forEach((element) {
      //   print(element.detail_desc);
      // });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          ...
          title: Text(widget.activityOwned.activity.activity_name)),
          ...
          children: [
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    ActivityDetailWidget(
                      type: 'text',
                      text: widget.activityOwned.activity.activity_description,
                    ),
                    (prov.isFetchingData)
            ...
            ElevatedButton(
              onPressed: (prov.isButtonDisabled)
                  ? () {}
                  : () {
                      prov.editActivityStatus(
                          widget.activityOwned.id, 'wait');
                    },
              child: Text('Mark As Done'),
              style: ElevatedButton.styleFrom(
                  primary:
                      (prov.isButtonDisabled) ? Colors.blue[400] : Colors.blue),
            ),
          ],
        ),
      )),
